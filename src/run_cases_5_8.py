...
def run_case_optimized(case_num, repo_dir, verbose=True, max_pulse_samples=200):
    ...
    t0 = time.time()
    offsets, z_spectrum, m0_val = simulate_case_optimized(
        model, seq_data, max_pulse_samples=max_pulse_samples, verbose=verbose)
    elapsed = time.time() - t0

    if verbose:
        print(f"\n  ✓ Done in {elapsed:.2f}s  "
              f"Z=[{z_spectrum.min():.4f}, {z_spectrum.max():.4f}]")

    return {'case': case_num, 'offsets': offsets, 'z_spectrum': z_spectrum,
            'm0_val': m0_val, 'model': model, 'defs': defs, 'elapsed': elapsed,
...
if __name__ == "__main__":
    ...
    if not REPO_DIR.exists():
        print(f"ERROR: BMsim_challenge not found at {REPO_DIR}")
        print("Please clone it: git clone https://github.com/pulseq-cest/BMsim_challenge.git")
        sys.exit(1)
